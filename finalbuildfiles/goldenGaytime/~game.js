...
class mainMenu extends Phaser.Scene {
  ...
  create() {
    ...
    this.input.once("pointerdown", function () {
      that.fnfetchAPI(window.trackingType + "PlayableStart");
...
class howtoplayGame extends Phaser.Scene {
  ...
  create() {
    ...
    var that = this;
    this.input.once("pointerdown", function () {
      that.fnfetchAPI(window.trackingType + "HowToPlay");
    ...
    this.time.delayedCall(0, () => {
      ...
      this.time.delayedCall(1000, () => {
        ...
        if (!this.isPlayableComplete) {
          ...
          this.time.delayedCall(1000, () => {
            that.fnfetchAPI(window.trackingType + "PlayableComplete");
            this.scene.start("endMenu");
          });
...
class endMenu extends Phaser.Scene {
  ...
  create() {
    ...
    bg.setOrigin(0.5);
    bg.setScale(0.45);
    this.crunch = this.sound.add("crunchMP3");
    // tween rotate bg
    this.tweens.add({
      targets: bg,
      rotation: 0.1,
