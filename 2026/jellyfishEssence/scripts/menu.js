...
class Menu extends Phaser.Scene {
  ...
  create() {
    ...
    this.bg = this.add.sprite(config.width * 0.5, config.height / 2, "bgg");
    this.bg.setScale(1.15);
    this.bg.flipX = false;
    this.bg.setAlpha(0);
    this.tweens.add({
      targets: this.bg,
    ...
    let char = this.add.sprite(config.width * 0.4 - 100, config.height * 1.1, "tvst");
    char.flipX = true;
    char.setAlpha(0);
    char.setScale(0.3).setOrigin(0.5, 1);
    ...
    this.input.on("pointerdown", (pointer) => {
      ...
      this.bg2 = this.add.sprite(config.width / 2, config.height * .4, "background");
      this.bg2.setScale(1).setAlpha(0);
