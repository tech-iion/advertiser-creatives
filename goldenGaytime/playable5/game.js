...
class mainMenu extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image("crumbs05", "assets/sprites/crumbs/Crumb_05.png");
    this.load.image("crumbs06", "assets/sprites/crumbs/Crumb_06.png");
    this.load.image("crumbs07", "assets/sprites/crumbs/Crumb_07.png");
    this.load.image("crumbs08", "assets/sprites/crumbs/Crumb_08.png");
    this.load.image("crumbs09", "assets/sprites/crumbs/Crumb_09.png");
    this.load.image("crumbs10", "assets/sprites/crumbs/Crumb_10.png");
  ...
  create() {
    ...
    //add crumbs randomly scattered on the edges without overlap
    /* const crumbPositions = []; // Track placed crumb positions
    const minDistance = 80; // Minimum distance between crumbs
    const maxAttempts = 15; // Reduced from 22 to avoid overcrowding

    for (let i = 1; i <= maxAttempts; i++) {
      // Try to place a crumb with overlap checking
      let attempts = 0;
      ...
      while (!validPosition && attempts < 10) {
        ...
        // Pick a random crumb sprite from 1-22
        const randomCrumbIndex = Math.floor(Math.random() * 14) + 1;

        // Randomly choose an edge (0=top, 1=right, 2=left) - excluding bottom
        const edges = [0, 1, 3]; // top, right, left only
        const edge = edges[Math.floor(Math.random() * 3)];
        const edgeMargin = 15 + Math.random() * 25; // Random margin between 15-40px

        if (edge === 0) {
          // Top edge
          x = Math.random() * (game.config.width - 2 * edgeMargin) + edgeMargin;
          y = edgeMargin + Math.random() * 15; // Slight random variation from edge
        } else if (edge === 1) {
          // Right edge
          x = game.config.width - edgeMargin - Math.random() * 15;
          y =
            Math.random() * (game.config.height - 2 * edgeMargin) + edgeMargin;
        } else {
          // Left edge (edge === 3)
          x = edgeMargin + Math.random() * 14;
          y =
            Math.random() * (game.config.height - 2 * edgeMargin) + edgeMargin;
        }

        // Check if this position overlaps with existing crumbs
        validPosition = true;
        for (let pos of crumbPositions) {
      ...
      // Only create crumb if we found a valid position
      if (validPosition) {
        const randomCrumbIndex = Math.floor(Math.random() * 14) + 1;
        var crumbs = this.add.image(
          0,
          0,
          `crumbs${randomCrumbIndex.toString().padStart(2, "0")}`
        );

        crumbs.x = x;
        crumbs.y = y;
        crumbs.setOrigin(0.5);
        crumbs.setScale(0.4 + Math.random() * 0.3); // Random size between 0.4-0.7
        crumbs.angle = Math.random() * 360; // Random rotation

        // Store this position to check against future crumbs
        crumbPositions.push({ x: x, y: y });

        // Start with alpha 0 and fade in
        crumbs.setAlpha(0);

        // Add alpha fade-in tween with random delay
        this.tweens.add({
          targets: crumbs,
          alpha: 0.7 + Math.random() * 0.3, // Final alpha between 0.7 and 1.0
          duration: 800 + Math.random() * 600, // Duration between 800-1400ms
          delay: Math.random() * 1500, // Random delay up to 1.5 seconds
          ease: "Power2.easeOut",
          onComplete: () => {
            // Random chance for continuous pulsing
            if (Math.random() > 0.6) {
              this.tweens.add({
                targets: crumbs,
                alpha: crumbs.alpha * 0.6,
...
class playGame extends Phaser.Scene {
  ...
  create() {
    ...
    this.stackarr = [
      "Keep stacking, size matters!",
      "Stack it like ya mean it!",
      "Keep goin', you're not knockin' off yet!",
      "That's what we're talkin' about!",
      "Now you're gettin' the hang of it!",
      "Slab it up, legend!",
      "You're on fire, keep it up!",
      "Don't stop now, you're killin' it!",
      "Stack 'em high, stack 'em proud!",
      "Look at you go, absolute unit!",
      "Keep pushin', no limits here!",
      "You've got this, stack master!",
      "That's the Gaytime way!",
      "Bigger is always better!",
      "Stack it 'til you make it!",
      "You're not done yet, are ya?",
      "Keep that momentum rollin'!",
      "Now that's a proper stack!",
      "Size it up, legend!",
      "Don't quit when it gets good!",
    ];
  ...
  showNotification() {
    ...
    const message = this.stackarr[this.randomIndex];
    this.randomIndex++;
